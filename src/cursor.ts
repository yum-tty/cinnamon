...
export function numericModeToCursorMode(mode: number): CursorMode {
  switch (mode) {
    case CursorBlink: return CursorMode.Blink
    case CursorStatic: return CursorMode.Static
    case CursorHide: return CursorMode.Hide
    default: return CursorMode.Blink
  }
}
...
export function SetMode(m: CursorModel, mode: CursorMode): [CursorModel, Cmd] {
  ...
  newMode.isBlinked = mode === CursorMode.Hide || !m.focused
...
export function Focus(m: CursorModel): [CursorModel, Cmd] {
  const newM = { ...m, focused: true, isBlinked: m.mode === CursorMode.Hide }

  if (m.mode === CursorMode.Blink && newM.focused) {
...
export function Update(m: CursorModel, msg: Msg): [CursorModel, Cmd] {
  ...
  switch (msg.type) {
    case "initialBlink":
      if (m.mode !== CursorMode.Blink || !m.focused) return [m, null]
      return [m, BlinkCmd(m)]
    ...
    case "blink": {
      ...
      const newM = { ...m, blinkTag: m.blinkTag + 1, isBlinked: !m.isBlinked }
      return [newM, BlinkCmd(newM)]
    }
...
export function BlinkCmd(m: CursorModel): Cmd {
  if (m.mode !== CursorMode.Blink) return null
