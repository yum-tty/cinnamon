...
export function New(): TextareaModel {
  const vp = Viewport({ width: DEFAULT_WIDTH, height: DEFAULT_HEIGHT })
...
function updateVirtualCursorStyle(m: TextareaModel): void {
  if (!m.useVirtualCursor) {
    SetMode(m.virtualCursor, CursorMode.Hide)
    return
  }
  ...
  if (m.styles.cursor.blink) {
    ...
    SetMode(m.virtualCursor, CursorMode.Blink)
    return
  }
  SetMode(m.virtualCursor, CursorMode.Static)
}
...
function wrapFn(lineArr: string, width: number): string[] {
  ...
  for (const r of lineArr) {
    ...
    if (spaces > 0) {
      const lineStr = lineParts[row]!.join("")
      const wordStr = wordParts.join("")
      if (getStringWidth(lineStr) + getStringWidth(wordStr) + spaces > width) {
        row++
        lineParts.push(wordParts)
        lineParts[row]!.push(" ".repeat(spaces))
        spaces = 0
        wordParts = []
      } else {
        lineParts[row]!.push(...wordParts)
        lineParts[row]!.push(" ".repeat(spaces))
        spaces = 0
        wordParts = []
      }
  ...
  if (getStringWidth(lineStr) + getStringWidth(wordStr) + spaces >= width) {
    lineParts.push([])
    lineParts[row + 1]!.push(...wordParts)
    spaces++
    lineParts[row + 1]!.push(" ".repeat(spaces))
  } else {
    lineParts[row]!.push(...wordParts)
    spaces++
    lineParts[row]!.push(" ".repeat(spaces))
  ...
  return lineParts.map(parts => parts.join(""))
}
...
function placeholderView(m: TextareaModel): string {
  ...
  for (let i = 0; i < m.height; i++) {
    ...
    if (m.showLineNumbers) {
      if (i === 0) {
        s.push(lineNumberView(m, 1, isLineNumber))
      } else if (isLineNumber) {
        s.push(lineNumberView(m, 0, isLineNumber))
      }
    }
...
function hardWrap(str: string, width: number): string {
  ...
  for (const line of lines) {
    ...
    for (const char of line) {
      ...
      current += char
      currentWidth += isWideChar(char.codePointAt(0) || 0) ? 2 : 1
    }
